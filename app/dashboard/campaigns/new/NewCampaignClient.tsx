        ...
        <div className="bg-gray-50 rounded-xl p-4 text-xs text-gray-500 space-y-1">
          ...
          {customVariableNames.length > 0 && (
            <>
              ...
              <p className="font-medium text-gray-600 mb-1">Custom variables</p>
              {customVariableNames.map((v) => (
                <div key={v} className="flex gap-3">
                  <code className="text-brand-600 w-24 flex-shrink-0">{`{${v}}`}</code>
          ...
          {customVariableNames.length === 0 && (
            <p className="text-gray-400 text-xs mt-1 italic">
              Add custom variables in Settings to use them here.
            </p>
          )}
        </div>
